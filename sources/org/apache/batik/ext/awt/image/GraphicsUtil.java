...
public class GraphicsUtil {
    ...
    public static void drawImage(Graphics2D g2d,
    	...
    	System.out.println("Height to change : " + cr.getTileHeight() + " Width : "+ cr.getTileWidth() + " y : " + cr.getMinY() + " x : " + cr.getMinX());
    	
        AffineTransform at = null;
        int tileHeight = cr.getHeight();
    	int tileWidth = cr.getWidth();
        while (true) {
        ...
        if (cr instanceof BufferedImageCachableRed) {
            ...
            if (g2dCS.equals(srcCM.getColorSpace()) &&
                ...
                // needs to changes by praveen for rendering team
                /*BufferedImage backUp = bicr.getBufferedImage();
                ResampleOp resampleOp = new ResampleOp(tileWidth, tileHeight);
                ResampleFilters filters = new ResampleFilters();
                resampleOp.setFilter(filters.getLanczos3Filter());
                BufferedImage resizedImage = new BufferedImage(tileWidth, tileHeight, backUp.getType());
                
                //Image resized = bicr.getBufferedImage().getScaledInstance(tileWidth, tileHeight, 16);
                
                BufferedImage toResize = bicr.getBufferedImage();
                
                resizedImage = resampleOp.doFilter(toResize, resizedImage, tileWidth, tileHeight);
                */
    ...
    public static Graphics2D createGraphics(BufferedImage bi,
    	...
    	System.out.println("Buffered Image : Height " + bi.getHeight() + " Width : " + bi.getWidth());
